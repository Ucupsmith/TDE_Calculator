...
const TdeeCalculatorPage = () => {
  ...
  const [hasCalculate, setHasCalculate] = useState<boolean>(false);
  const [buttonClicked, setButtonClicked] = useState<boolean>(false);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [hasCalculateModal, setHasCalculateModal] = useState<boolean>(false);
  ...
  const { setTdeeId, tdeeId } = useTdee();
  ...
  const handleSaveTdee = useCallback(async () => {
    ...
    try {
      ...
      if (response) {
        ...
        localStorage.setItem('hasCalculateTdee', 'true');
        setCalculateTdee(response);
        setHasCalculate(true);
        setHasCalculateModal(true);
      }
      return response;
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const calculated = localStorage.getItem('hasCalculateTdee');
    setHasCalculate(Boolean(calculated));
    setHasCalculateModal(Boolean(calculated));
  }, []);
          ...
          {hasCalculateModal && (
            <div className='fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60'>
              <div className='bg-white rounded-lg p-6 text-center'>
                <Typography className='text-lg font-bold mb-4 text-black'>
                  Kamu sudah pernah calculate TDEE!
                </Typography>
                <Typography className='mb-6 text-black'>
                  Mau hitung ulang atau pakai hasil sebelumnya?
                </Typography>
                <div className='flex justify-center gap-4'>
                  <Button
                    onClick={() => {
                      setHasCalculate(false);
                      setHasCalculateModal(false);
                      localStorage.removeItem('hasCalculatedTDEE');
                      setCalculateTdee(null);
                    }}
